...
my_ctl_functionOverrides="configTomcatSSLServerKey installCertificates configShibbolethFederationValidationKey performStepsForShibbolethUpgradeIfRequired askForSaveConfigToLocalDisk patchShibbolethLDAPLoginConfigs doInstall"
...
# patch shibboleth config files
	...
	cp ${Spath}/files/InCommon/attribute-filter.xml.template /opt/shibboleth-idp/conf/attribute-filter.xml
...
#FIXME: alter override for federation aware setting rather than just the 'CAF' one
	...
	if [ "${type}" = "ldap" ]; then
		...
		cat ${Spath}/${prep}/InCommon/login.conf.diff.template \
			...
			> ${Spath}/${prep}/InCommon/login.conf.diff
		files="`${Echo} ${files}` ${Spath}/${prep}/InCommon/login.conf.diff"
		patch /opt/shibboleth-idp/conf/login.config -i ${Spath}/${prep}/InCommon/login.conf.diff >> ${statusFile} 2>&1
	fi
